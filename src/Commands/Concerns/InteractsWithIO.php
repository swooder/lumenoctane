...
trait InteractsWithIO
{
    ...
    public function requestInfo($request, $verbosity = null)
    {
        ...
        $this->output->writeln(sprintf(
           '  <fg=%s;options=bold>%s </>   <fg=cyan;options=bold>%s</> <options=bold>%s</><fg=green> %s%s%s ms</>',
           $color,
           $statusCode,
           $method,
           $url,
    ...
    public function handleStream($stream, $verbosity = null)
    {
        switch ($stream['type']) {
            case 'request' :
                $this->requestInfo($stream, $verbosity);
                break;
            case 'throwable':
                $this->throwableInfo($stream, $verbosity);
                break;
            case 'shutdown':
                $this->shutdownInfo($stream, $verbosity);
                break;
            default :
                $this->info(json_encode($stream, $verbosity));
                break;
        }
//
